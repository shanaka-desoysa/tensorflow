...
def combine_inputs(X):
    with tf.name_scope("combine_inputs"):
        ...
        #if DEBUG:
        #    return_val = tf.Print(return_val, [return_val, tf.shape(return_val)], "combine_inputs = ", summarize=10)
        return return_val


def inference(X):
    with tf.name_scope("inference"):
        return_val = tf.nn.softmax(combine_inputs(X))
        if DEBUG:
            return_val = tf.Print(return_val, [return_val, tf.shape(return_val)], "inference = ", summarize=10)
        return return_val
...
def inputs():
    with tf.name_scope("inputs"):
        sepal_length, sepal_width, petal_length, petal_width, label =\
            read_csv(2, DATA_FILE, [[0.0], [0.0], [0.0], [0.0], [""]])
        ...
        #    label_number = tf.Print(label_number, [label, label_number, tf.shape(label_number)], "label_number = ")
        #    features = tf.Print(features, [features, tf.shape(features)], "features = ", summarize=5)
...
def evaluate(sess, X, Y):
    with tf.name_scope("evaluate"):
        ...
        if DEBUG:
            predicted = tf.Print(predicted, [predicted, tf.shape(predicted)], "predicted = ", summarize=10)
...
with graph.as_default():
    ...
    X = tf.Print(X, [X, tf.shape(X)], "X = ", summarize=10)
    W = tf.Print(W, [W, tf.shape(W)], "W = ", summarize=10)
    b = tf.Print(b, [b, tf.shape(b)], "b = ", summarize=10)
